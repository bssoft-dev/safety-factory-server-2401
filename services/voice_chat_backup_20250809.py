...
class VoiceChat(AudioUtils):
            ...
            if time_interval >= self.SYNC_INTERVAL:
                ...
                if len(active_clients) > 0:
                    ...
                    if self.do_stt:
                        for ws_idx, client_id in active_clients:
                            # 각 클라이언트별로 STT 처리
                            client_audio = [combined_audio_int16[active_clients.index((ws_idx, client_id))]]
                            asyncio.create_task(self.stt_audio(client_audio, room_name, client_id))
    ...
    async def stt_audio(self, combined_audio_int16, room_name: str, client_id: int):
        try:
            # 발화자 정보 가져오기
            worker = self.client_info[client_id]["person_name"]
            
            # 오디오 데이터를 바이트로 변환
            audio_bytes = np.concatenate(combined_audio_int16).astype(np.int16).tobytes()
